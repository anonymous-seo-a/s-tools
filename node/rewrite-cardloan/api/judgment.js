...
// ─────────────────────────────────────────────────────────────
// rewrite-cardloan フォーク (2026-07-29): cardloan 専用システム。
//   cardloan 以外のジャンルは全入口で 403 (securities 等は本家 rewrite が担当)。
//   auto-batch は Daiki 決定 (2026-07-28) により無効で開始 (誤起動再発防止)。
// ─────────────────────────────────────────────────────────────
const ALLOWED_GENRE = 'cardloan';
const AUTO_BATCH_ENABLED = false;
const DISABLED_REWRITE_GENRES = { has: (g) => g !== ALLOWED_GENRE };
...
// safe-cell 学習緩和は cardloan では無効 (Daiki 決定 2026-07-28: 最厳格で開始。
// 運用実績を見て再有効化する場合はこのフラグのみ変更)。
const SAFE_CELL_LEARNING_ENABLED = false;

function autoJudgeSession(sessionId) {
  const conn = open();
  const { safe: safeCells, learned } = SAFE_CELL_LEARNING_ENABLED
    ? learnSafeCells(conn)
    : { safe: new Set(), learned: [] };
  const diffs = conn.prepare(
...
async function applySessionCore(id, { dryRun = false } = {}) {
  ...
  // ─── L3 最終ゲート (keeper-bridge) ─────────────────────────
  // 適用後の本文全体を cardloan-keeper の公式規制 + 法令原文に照らして検査。
  // high/medium 違反 or ゲート実行不能 = 適用中断 (fail-closed)。low は警告として通す。
  // 緊急回避は KEEPER_GATE_SKIP=1 (Daiki 判断のみ)。
  {
    const gateText = keeperBridge.rawToPlainText(plan.raw);
    const gateProducts = keeperBridge.detectProducts(gateText).map((p) => p.product_id);
    let gate;
    try {
      gate = keeperBridge.checkArticleGate(gateProducts, gateText);
    } catch (e) {
      const err = new Error(`apply中断: L3規制ゲート実行不能 (fail-closed) — ${e.message}`);
      err.httpStatus = 502;
      throw err;
    }
    if (!gate.ok) {
      const detail = gate.blocking.map((v) => `[${v.severity}]「${(v.excerpt || '').slice(0, 40)}」${v.source}: ${(v.why || '').slice(0, 60)}`).join(' / ');
      const err = new Error(`apply中断: L3規制ゲート違反 ${gate.blocking.length}件 (照合${gate.rules_checked}ルール) — ${detail}`);
      err.httpStatus = 422;
      err.gate = gate;
      throw err;
    }
    for (const v of gate.violations.filter((x) => x.severity === 'low')) {
      skipped.push({ diff_id: null, reason: `L3警告(low):「${(v.excerpt || '').slice(0, 40)}」${(v.why || '').slice(0, 60)}` });
    }
    console.log(`[applySessionCore] L3ゲート通過: 商材=${gateProducts.join(',') || '(検出なし)'} 照合${gate.rules_checked}ルール 違反0(blocking)`);
  }
  ...
  router.post('/auto-batch', async (req, res) => {
    try {
      if (!AUTO_BATCH_ENABLED) {
        return res.status(403).json({ error: 'auto-batch は rewrite-cardloan では無効です (Daiki 決定 2026-07-28: 検証完了まで手動判定のみ)' });
      }
      const busy = isGenerationBusy();
      if (busy) return res.status(409).json({ error: busy });
