...
実測 (人間執筆記事 securities/4185): 段落=1〜2文・平均65字(max141), 視覚要素=見出しの85%, 太字=5個/1000字。

1. 段落分割のリズム (改行欠落の根治・ただし入れすぎない)
   - **1段落 = 1〜2文** を目安に <p> を区切る。実測で soico 記事の段落は 1文48% / 2文49% / 3文以上は0%。
   - **3文以上を1つの <p> に詰め込まない (本文の壁の禁止)**。文の途中では切らない。
   - 1〜2文でまとまる内容はムリに1文ずつに割らない (改行過剰は house style から外れる)。
   - 各 <p> は独立した Gutenberg ブロックとして出力 (HTML改行 <br> や空段落 <p>&nbsp;</p> は使わない。
     ブロック間の余白はテーマの段落マージンで付くため、空行ブロックを自分で挟まない)。
   - 良い例 (1〜2文で1段落):
     <p>松井証券は、長期の資産形成に役立つiDeCo（個人型確定拠出年金）も取り扱っています。</p>
     <p>iDeCoは掛金が全額所得控除の対象となり、運用益も非課税で再投資される私的年金制度です。</p>
   - 悪い例 (3文以上を1段落に詰める・禁止):
     <p>松井証券はiDeCoを取り扱っています。iDeCoは掛金が全額所得控除の対象です。運用益も非課税で再投資されます。</p>
...
target_query (Q[i]): ${target_query}`);
  ...
  {
    const articleText = renderArticleView(article_view);
    const pids = keeperBridge.detectProducts(`${title}\n${articleText}`).map((p) => p.product_id);
