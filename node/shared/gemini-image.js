...
function buildEyecatchPrompt({ title, contentSummary, genre }) {
  ...
  // 注意: "SEO"/"アフィリエイト"/"アイキャッチ" 等の語をプロンプトに書くと、画像生成モデルが
  // それを画像内テキストとして崩れた形で描画してしまう (priming)。そのため、これらの語を
  // プロンプトから完全に排除し、中立的な「記事のヘッダー / メインビジュアル」表現で指示する。
  return [
    '日本の金融メディアの記事トップに掲載する、記事内容を表すメインビジュアル（ヘッダー画像）を 16:9 のアスペクト比で作成してください。',
    '',
    `【ジャンル】${theme}`,
    `【記事タイトル】${title}`,
    ...
    '- 背景は濃紺〜ブルー基調で、信頼感・清潔感のある配色',
    '- 記事の要点を表す「短い見出し」を主役にする。タイトルを長文のまま全部入れず、最重要キーワードだけ大きく、数字や訴求語はアクセントカラー（黄/オレンジ/赤等）で強調',
    '- 要点を表す小さなチップ/バッジ（2〜4個）や「2026年最新」等の年バッジを添えて情報量を持たせる',
    '- 左にテキスト、右にイラストのような明快な構図。余白と階層を意識した完成度の高いデザイン',
    '',
    '【画像内の文字について（厳守）】',
    '- 画像に入れる文字は、記事の見出し・要点キーワード・数字・年バッジ・チップのテキストのみ',
    'この指示文に出てくる作業用の語（「ジャンル」「記事」「内容」「ヘッダー」「メインビジュアル」やジャンル名ラベル等）は画像内の文字として描画しない',
    '- 日本語は正確に。文字の重複・崩れ・意味不明な文字列・スペルミスを入れない。同じ語句を繰り返さない。読みやすいフォントと高コントラスト',
