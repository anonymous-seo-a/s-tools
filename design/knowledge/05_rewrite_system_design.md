...
│   ├── shared/                           ← (γ) lazy 構築方針
│   │   ├── llm-adapters/
│   │   │   └── anthropic-adapter.js      ← Part 3 (Sonnet 4.6 + Opus 4.7)
│   │   │   (※ openai/gemini-adapter は必要時に追加)
│   │   ├── schemas/
│   │   │   └── intent_dimension.schema.json  ← Part 3 (JTBD 5 次元 ajv)
│   │   ├── serpapi-adapter.js            ← Part 4 (SerpApi 公式 npm ラッパ)
│   │   ├── wp-structured.js              ← Part 5 中間案
│   │   │                                    (案C 着手時に監修者除去/ez-toc/CTA 拡張)
│   │   └── archive-adapters/             ← Phase 3 で追加予定
│   │       ├── wayback-adapter.js
│   │       └── archive-today-adapter.js
│   ├── (既存 monitor 系: monitor-collectors.js, monitor-jobs.js 等)
│   └── rewrite/                          ← Phase 4 実装本体
│       ├── target-selection/             ← Phase 1 確立 (4 軸統合済)
│       ├── query-fanout/                 ← Part 3 (Step A-2)
│       ├── post-target-query/            ← Part 4 (案A GSC 統合)
│       ├── competitor-corpus/            ← Part 4 (Step A-1 SerpApi)
│       ├── fact-set/                     ← Part 5 (Step A-1 後半)
│       │   ├── extract.js                  (master_fact_set + UPDATE)
│       │   └── ig-score.js                 (master_information_gain_score)
│       ├── api/                          ← Phase 1 (queue.js)
│       ├── batch/                        ← Phase 1 (daily-target-selection)
│       ├── scripts/                      ← CLI runners + smoke tests
│       ├── llm-execution/                ← 工程6' 案C (Phase 2 後半)
│       └── ab-testing/                   ← 工程8 Step A-4 (Phase 3)
└── data/
...
shared/ 構築履歴:
  Part 3: anthropic-adapter.js + intent_dimension.schema.json
  Part 4: serpapi-adapter.js
  Part 5: wp-structured.js (中間案)
  
削減判定 (元設計から最小性で削減):
  llm-adapters/common.js               ← SDK 内部 retry maxRetries=5 で代替
  llm-adapters/adapter-interface.js    ← anthropic-adapter シグネチャが暗黙 I/F
  google-adapter.js                    ← rewrite/post-target-query/build.js inline で完結
  article-context.js                   ← 案C LLM 実行レイヤー着手時に判断
  wp-rest-client.js                    ← rewrite/fact-set/extract.js inline で完結
                                          (WP_API_* 系の新キー名と既存 SITE_URL/WP_USERNAME
                                           系の不整合を回避、monitor 系統合は別案件)
```
...
#### 実装状態 (Part 5 完成、smoke pass)

```
master_fact_set:
  実装: node/rewrite/fact-set/extract.js
  方式: LLM 1 コール / 記事で Layer 1〜3 同時抽出 (Sonnet 4.6)
  上限: layer1=10 / layer2=15 / layer3=10 (LLM 指示 + コード両方で固定)
  入力: タイトル + plain_text、上限 30K 文字 (冒頭切り捨て)
  YMYL: プロンプトで違反主張除外明示 (警戒バイアス [16] 上流フィルタ怠惰対処)

master_competitor_corpus.fact_set_snapshot:
  実装: 同 extract.js 内、JSON で保持
  形式: { layer1: [...], layer2: [...], layer3: [...], extracted_at: ISO }
  knowledge/05 設計の post_id NOT NULL 制約に従い、競合 fact は corpus 側に集約

master_information_gain_score:
  実装: node/rewrite/fact-set/ig-score.js
  方式: 完全一致差集合 (trim + lowercase 最小正規化)
  IG Score = gap_count として実装、raw deltas は notes JSON に保存
  notes JSON: { self_fact_count, competitor_union_count, gap_fact_samples,
                layer3_gap_count, calculation_method }

smoke 検証 (post_id=7170 × query_fanout_id=11):
  ...
  elapsed: 8ms (LLM コール不要、純 SQL + Set 演算)

shared/ 構築:
  shared/wp-structured.js (中間案):
    extractSelfArticle(html): h タグ階層 + 段落/li 連結 sections
    extractCompetitorContent(html): cheerio.text() フォールバック + nav/footer 除去
    案C 着手時に拡張予定: 監修者除去 / ez-toc 除去 / CTA マーカー化 / 82% 削減実測

拡張余地 (Phase 2 後半):
  - 表現揺れ吸収 (意味的一致): LLM 判定 or embedding 類似度
  - log/正規化版 IG Score (raw deltas notes JSON から再計算可能)
  - 一括投入バッチ (全 cardloan 434 記事 × 全 Layer1 sub_query 10 件)
```
...
| カテゴリ | 番号 | 件数 |
|---|---|---|
| A. 設計判断バイアス | [1][2][3][4][5] | 5 |
| B. 実装過剰バイアス | [6][9][10][11][12][14][20] | 7 |
...
#### [6] UI 仕様の過剰精緻化バイアス (Phase 3 で警戒指示)
```
UI 仕様を Phase 3 で精緻化しすぎると Phase 4 実装で再調整コストが嵩む
→ Phase 4 着手時は MVP 最小実装 (既存 CSS 流用、新規 CSS 0 行) を維持
→ Phase 4 Part 1 で回避成功
```

#### [9] LLM プロンプト過剰精緻化バイアス (Phase 4 Part 3 で発動)
```
プロンプト最適化を初期実装段階で完結しようとする傾向
→ 初期は「動く」レベルで OK、最適化は実運用フェーズ
→ Part 3 / Part 5 fact 抽出プロンプトで継続適用、回避成功
```

#### [10] JSON Schema 過剰汎用化バイアス (Phase 4 Part 3 で発動)
```
JSON Schema を未来の拡張のために汎用化しすぎる
→ ajv 設定最小限、custom formats なし、null 許容、空配列許容
→ intent_dimension.schema.json / fact_set_snapshot で継続適用
```

#### [11] Adapter 層の過剰抽象化バイアス (Phase 4 Part 3 で発動)
```
3 社対応 (Anthropic/OpenAI/Gemini) や 4 source 対応のために抽象化を過剰にする
→ 単一実装、I/F 抽象化遅延、必要時に拡張
→ anthropic-adapter / serpapi-adapter / wp-structured 全件で継続適用
```

#### [12] スケルトン作成隠れたコストバイアス (Phase 4 Part 3 で発動、handoff Part 2 でも観察)
```
空ファイルだけ作って後で実装する設計が、後段の戻し時に大きなロストになる
→ 必要関数のみ export、スケルトンファイル作成しない
→ shared/llm-adapters/common.js / adapter-interface.js 削減判定で適用
→ shared/wp-structured.js も「中間案で必要機能のみ」適用 (Part 5)
```

#### [14] 細分化暴走バイアス (Phase 4 Part 3 で発動)
```
LLM 抽出を細分化 (per layer / per type) するとコスト × N、デバッグ範囲拡大
→ 上限 N 固定、1 コール / 記事 で完結
→ Part 3 Layer1 主題分解 (上限 7、平均 6.2) / Part 5 fact 抽出 (Layer 1〜3 同時、上限 10/15/10)
```

#### [20] fact 抽出網羅性追求バイアス (Phase 4 Part 5 で警戒)
```
「全 fact」を抽出しようとする誘惑
→ 上限 N 固定 (LLM 指示 + コード両方)、重要度上位 N 件のみ
→ Part 5 fact 抽出で適用、smoke pass
```

### C. プロンプト/LLM 整合バイアス

#### [13] Google fan-out 正解探求バイアス (Phase 4 Part 3 で警戒)
```
「Google 内部 fan-out と一致させたい」探求心が動作するが、外部から再現不可能
→ 「Google 内部 fan-out 一致不要、Daiki システムの目的は YMYL コンテンツ品質向上」を明示
→ Part 3 Layer1 主題分解で適用、回避成功
```

#### [15] intent_dimension 自動生成期待バイアス (Phase 4 Part 3 で警戒)
```
JTBD 5 次元すべてを LLM で自動生成できる前提
→ null 許容、barrier 69% null 率で機能確認、抽出不能なら null
→ 質的 fallback (人手記入) を spec で許容
```

#### [16] YMYL 上流フィルタ怠惰バイアス (Phase 4 Part 3 で新規確立)
```
...
→ Phase 4 Part 5 fact 抽出プロンプトでも継続適用、回避成功
→ Phase 2 後半 (案C LLM 実行レイヤー) でも継続適用必須
```

#### [21] LLM 出力構造化保証バイアス (Phase 4 Part 5 で警戒)
```
JSON Schema 検証で全件 pass を目指すと過度に厳格化
→ Array.isArray チェック後 slice、null 許容、空配列許容
→ JSON パース失敗は warn 継続、ajv で全件 pass を目指さない
→ Part 5 fact 抽出で適用、smoke pass
```

### D. 指示解釈・判断委任境界バイアス

#### [7] Daiki 指示の literal vs intent 取り違えバイアス (Phase 4 Part 1 で発動)
```
発動箇所: 軸1 placeholder の path 指定で literal 優先
発動結果: target-selection/ vs scoring/ のパス不整合発生
対処: Phase 4 Part 2 で全 axis を target-selection/ に統一する refactor

→ Daiki 指示が literal vs intent のどちらを意図しているか曖昧な場合は確認する
```

#### [8] schema 変更の判断委任境界バイアス (Phase 4 Part 1 で発動)
```
発動箇所: NOT NULL 解除を Daiki 戻し条件にせず Claude 判断で進めた
発動結果: 意図 (NULL = 未計算) が明確だったため正解だが、戻すべきだった境界線

→ schema 変更は明確な意図共有がない限り Daiki 確認、判断委任の境界として記憶
```

### E. 外部 API/運用整合バイアス

#### [17] SerpApi コスト浪費バイアス (Phase 4 Part 4 で警戒)
```
dev で気軽に叩くと本番運用時に枯渇リスク
→ smoke 累計 2 クエリ (cache 効果で実消費 1〜2)、月次 0.04% 消費
→ shared/serpapi-adapter.js に dev/prod 環境変数で動作切替を最初から組込む
```

#### [18] 取得対象範囲拡大バイアス (Phase 4 Part 4 で警戒)
```
SerpApi の 4 source_type (organic/paa/related_search/ai_overview_citation) を
全投入したい誘惑
→ master_competitor_corpus は organic top N=3 のみ、PAA/related は serp_features 集計のみ
→ 案B # 4 PAA/related → master_query_fanout 投入は別タスクとして encode
```

#### [19] 認証情報 Git 混入リスクバイアス (Phase 4 Part 4 で警戒)
```
service-account-key.json や .env を git に混入させるリスク
→ ルート .gitignore で *-key.json / .env / *.env 完全カバー
→ Daiki 配置作業時に typo (service-account-key..json) 検出 → ide_opened_file 通知活用
```

#### [22] 環境変数値構造仮定バイアス (Phase 4 Part 5 で新規確立)
```
発動箇所: extract.js fetchWpContent() smoke 実行時 → WP REST 404 rest_no_route
発動内容: WP_API_BASE_URL=https://soico.jp/no1/wp-json/wp/v2 (フルパス) を
         subdir のみと仮定して実装、/wp-json/wp/v2 二重付加発生

派生事項:
  - GSC_PROPERTY_URL の www あり/なし不整合 (Part 4)
  - 既存 monitor 系の旧キー名 (SITE_URL / WP_USERNAME) と新キー名 (WP_API_*) の違い

対処パターン:
  1. node -e で実値を確認 (redact 不要、ローカル環境)
  2. 含有判定で吸収する設計 (例: /\/wp-json\/wp\/v\d+/.test(raw))
  3. shared/ 系の env 参照ロジックは「含有判定で吸収」を推奨
...
| [3] | [3] | 強推奨ラベルへの追従 |
| [4] | [4] | 機能を盛りたくなる |
| [5] | [5] | テーブル単位 vs システム全体最小性 |
| (旧 [6]) | [6] | UI 仕様の過剰精緻化 |
| (旧 [7]) | [7] | Daiki 指示 literal vs intent |
| 新規 | [8] | schema 変更の判断委任境界 |
| [a] | [9] | LLM プロンプト過剰精緻化 |
| [b] | [10] | JSON Schema 過剰汎用化 |
| [c] | [11] | Adapter 層の過剰抽象化 |
| [d] | [12] | スケルトン作成隠れたコスト |
| [e] | [13] | Google fan-out 正解探求 |
| [f] | [14] | 細分化暴走 |
| [g] | [15] | intent_dimension 自動生成期待 |
| [h] | [16] | YMYL 上流フィルタ怠惰 |
| [i] | [17] | SerpApi コスト浪費 |
| [j] | [18] | 取得対象範囲拡大 |
| [k] | [19] | 認証情報 Git 混入リスク |
...
→ 警戒バイアス [22] 環境変数値構造仮定として XIV 章に登録
→ shared/ 系で env 参照する全コードに適用
```
...
[完了] Phase 4 Part 5（Step A-1 完成、2026-05-05 Part 5）
       - shared/wp-structured.js: cheerio 構造化抽出 中間案 (4 件目 lazy 構築)
       - rewrite/fact-set/extract.js: master_fact_set Layer 1〜3 LLM 抽出
       - rewrite/fact-set/ig-score.js: 完全一致差集合 + raw deltas notes JSON
       - master_fact_set: 25 件 (post_id=7170)
       - master_competitor_corpus.fact_set_snapshot: 3 件 UPDATE
       - master_information_gain_score: 1 件 (Layer 1〜3 gap = 23 / 45 / 14)
       - 戻し対応: WP_API_BASE_URL 値構造仮定問題、smoke 即修正
       - 観察事項: cardloan Layer 3 限定性 / Layer 2 表現揺れ問題
       - 警戒バイアス [22] 環境変数値構造仮定 を新規確立
       - 警戒バイアス [1]〜[22] 通し番号統合完了 (XIV 章)
       - Phase 2 主要実装タスク 4.5/7 達成

次の作業: Phase 4 MVP Phase 2 残 2.5 タスク
  推奨第一手 (Part 5 末で encode):
    優先1: master_hcu_checklist (案B # 5、軽量・独立)
           → 依存なし、LLM のみで完結、本日確立 shared/llm-adapters/ 応用
           → 工数 1〜2 日、Phase 2 主要 5/7 達成
    優先2: master_article_similarity α (案B # 9、軽量・独立)
           → 依存なし、TF-IDF/Embedding 独立
