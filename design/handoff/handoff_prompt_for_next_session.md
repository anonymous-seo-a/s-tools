...
最終更新: 2026年5月5日 Part 3（Step A-2 完了、Phase 2 主要タスク 1/7 達成）
前提環境: Claude Code（s-tools/design/ 配下に全資産統合済み、s-tools/node/rewrite/ に Phase 1 実装 + Part 2 残課題消化済み + Part 3 Step A-2 実装済み、shared/ (γ) lazy 構築開始）
...
## ★ Daiki 確定優先順位 (2026-05-05 Part 3 末で確定)
...
[完了] ★ Step A-2 Query Fan-out 二段構造 (Part 3、5 コミット)
       - shared/llm-adapters/anthropic-adapter.js (γ lazy 構築検証成功)
       - master_query_fanout: layer=1 (主題分解) 10件 + layer=2 (micro-intent) 62件
       - intent_dimension JTBD 5次元 JSON Schema 実装 + ajv validation
       - 警戒バイアス [h] YMYL 上流フィルタ怠惰 を新規確立 (即時対応)

[次] ★ Phase 2 残 6 タスクから着手判断 (Part 3 末で 3 案推奨)
     優先1: 案A (Google API 整備) を Daiki が認証情報準備して着手
            → Step A-1 (master_competitor_corpus / fact_set / IG Score) の文脈充実の前提
     ...
     優先2: タスク 2 SerpApi Adapter + master_competitor_corpus
            ...
            → 本日確立した shared/llm-adapters/ パターンを serpapi-adapter.js に展開
     優先3: タスク 5 master_hcu_checklist + LLM 自動評価 (案B # 5)
            → 軽量、独立、重実装の合間に挟む候補
...
| # | タスク | 依存 | 状態 | 推奨度 |
|---|---|---|---|---|
| 1 | master_post_target_query 構築 (案D 2.A) | 案A (Google API) | 案A 待ち or 手動投入 | 中 |
| 2 | master_competitor_corpus 投入 (Step A-1) | SerpApi Adapter | 着手可能 | 高 |
| 3 | master_fact_set 投入 (Step A-1) | wp-structured.js | 着手可能 | 中 |
| 4 | master_information_gain_score 計算 (Step A-1) | 2 + 3 完了 | 1〜3 完了後 | 中 |
| 5 | master_hcu_checklist + LLM 自動評価 (案B # 5) | なし | 着手可能、軽量 | 高 |
| 6 | master_article_similarity α (案B # 9) | なし | 着手可能 | 低 |
...
Part 3 で構築済み (A-2 範囲、最小化済み):
  shared/llm-adapters/anthropic-adapter.js  ← Sonnet 4.6 + Opus 4.7 ラッパ
  shared/schemas/intent_dimension.schema.json ← JTBD 5次元 JSON Schema (ajv 用)

Part 3 で削減判定 (元設計から最小性で削減):
  shared/llm-adapters/common.js             ← SDK 内部 retry maxRetries=5 で代替
                                              論点2-1 整合、後段 LLM 追加時に再評価
  shared/llm-adapters/adapter-interface.js  ← anthropic-adapter シグネチャが暗黙 I/F
                                              OpenAI/Gemini 追加時に形式化 (YAGNI)

次セッション着手時に追加が想定される shared/ 構成:
  shared/serpapi-adapter.js                 ← SerpApi 連携 (タスク2 着手時)
  shared/wp-structured.js                   ← cheerio 構造化抽出 (案C LLM 実行レイヤー時)
  shared/article-context.js                 ← コンテキスト集約 (同上)
...
# 最初のタスク (Daiki 確定優先順位に従う、Part 3 末で encode)
...
2. 直近のセッション記録を読み、進行スタイルと判断構造を吸収:
   ...
   - sessions/2026-05-05_part3_step_a2.md (Part 3、最新、Step A-2 完了総括)
3. Part 3 末状態 + Daiki 確定優先順位を Daiki に確認
4. ★ Step 1: 次の作業 3 案を Daiki に提示 + 推奨提示
   (A) タスク 2 SerpApi Adapter + master_competitor_corpus 着手 (Step A-1 系列、推奨)
       - shared/serpapi-adapter.js を (γ) lazy 構築
       - SerpApi (Starter $25/月) で実 SERP 取得 → master_competitor_corpus 投入
       - Top N=3 (デフォルト)、cardloan 上位記事の競合構造取得
       - 本日確立した shared/llm-adapters/ パターンを横展開
       - 工数感: 1〜2セッション
   (B) タスク 5 master_hcu_checklist + LLM 自動評価 着手 (案B # 5、軽量)
       - HCU 22 項目を Google 公式から取得 (現時点では Phase 1 リサーチ時点の値)
       - LLM (Sonnet 4.6) で per-post 評価 → master_hcu_checklist に投入
       - 独立タスク、依存なし
       - 工数感: 1セッション
   (C) 案A (Google API 整備) を Daiki が認証情報準備して着手
       - 認証情報準備は Daiki 環境作業、Claude 自走不可
       - GSC max_impression / max_clicks 取得経路を起動
       - master_post_target_query 構築の前提

   推奨理由 (Part 3 末で記録):
   - (A) は Step A-1 系列の起点、shared/ lazy 構築方針の本格検証 (continued)
   - (A) は Step A-2 で確立した 3 者協働ループのパターンを継続適用可能
   - (B) は (A) と独立、軽量タスクとして並行 or 前後着手可能
   - (C) は Daiki 環境作業のため、(A) または (B) を進めながら並行
